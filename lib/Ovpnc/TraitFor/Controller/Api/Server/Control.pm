        ...
        unless ( $self->_check_running ) {
            ...
            if ( -e $self->openvpn_pid and -r $self->openvpn_pid ) {
				...
				if ( $_file_pid == $_pid ){
					...
					# Report the error
					return { 
						error => 'Found two different pid numbers: Active: ' . $_pid
								. ' and from pid file: ' . $_file_pid
								. '. Something has probably gone wrong while starting up'
					}
				}
            } else {
            	return { error => 'Server failed to create pid file and/or startup!' };
            }
        }
    }
}
...
sub stop {
    ...
    if ( $_pid ){
        ...
        kill 9, $_pid or return { error => "Cannot kill OpenVPN with pid " . $_pid };
